...
def beautify_js_file(js_path: str):
    """
    使用 jsbeautifier 自动美化 JS 文件
    """
    opts = jsbeautifier.default_options()
    opts.indent_size = 2
    opts.preserve_newlines = True

    logger.info(f"Beautifying JS file: {js_path}")
    ...
    output_path = js_path.replace(".js", "_beautified.js")
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(beautified_code)

    logger.info(f"Beautified file saved as: {output_path}")
...
def export_data(
    ...
    if output_file_path.lower().endswith('.js'):
        beautify_js_file(output_file_path)
